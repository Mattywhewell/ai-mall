-- rls-fix-remaining.sql
-- Patch to fix remaining policies (ensure auth.uid() is cast to UUID and check/qual are present)
BEGIN;
...
CREATE POLICY "Users can send messages" ON chat_messages
  FOR INSERT WITH CHECK (conversation_id IN (SELECT id FROM chat_conversations WHERE (user_id IS NOT NULL AND auth.uid() IS NOT NULL AND user_id = auth.uid()::uuid) OR (user_id IS NULL AND session_id IS NOT NULL)));

DROP POLICY IF EXISTS "Users can view messages in their conversations" ON chat_messages;
CREATE POLICY "Users can view messages in their conversations" ON chat_messages
  FOR SELECT USING (conversation_id IN ( SELECT id FROM chat_conversations WHERE ((user_id IS NOT NULL AND auth.uid() IS NOT NULL AND user_id = auth.uid()::uuid) OR (user_id IS NULL AND session_id IS NOT NULL))));
...
-- Product reviews: update/delete already exist but ensure casting
...
-- Select any policies where auth.uid() appears but not auth.uid()::uuid
