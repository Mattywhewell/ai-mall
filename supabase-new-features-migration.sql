...
-- ============================================
-- GIFT REGISTRIES TABLE (moved up for FK dependency)
-- ============================================
...
-- ============================================
-- GIFT REGISTRY SHARES TABLE
-- ============================================
CREATE TABLE IF NOT EXISTS gift_registry_shares (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  ...
  UNIQUE(registry_id, user_id, email)
);

CREATE INDEX IF NOT EXISTS idx_gift_registry_shares_registry_id ON gift_registry_shares(registry_id);
CREATE INDEX IF NOT EXISTS idx_gift_registry_shares_user_id ON gift_registry_shares(user_id);
CREATE INDEX IF NOT EXISTS idx_gift_registry_shares_email ON gift_registry_shares(email);

-- New Features Database Migration
-- Includes: Wishlists, Reviews, Notifications, Followers, Comparisons
...
-- ============================================
-- Add additional fields to user_profiles if not exist
...
BEGIN
  ...
  IF NEW.price < OLD.price THEN
    -- Notify all users who have this in wishlist
...
CREATE POLICY "Users can view own order gift options" ON order_gift_options
  FOR SELECT USING (true); -- Simplified for now, update based on orders table schema
...
CREATE POLICY "Shared users can view registry items" ON gift_registry_items
  FOR SELECT USING (
    ...
    OR
    (
      -- By email (if email is available in JWT)
...
COMMENT ON TABLE gift_registry_shares IS 'Tracks users/emails invited to access a gift registry.';
