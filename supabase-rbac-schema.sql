...
-- Add audit fields to pending_products table
...
CREATE TRIGGER pending_products_audit_trigger
...
CREATE POLICY "Admins can view all pending products"
  ON pending_products FOR SELECT
  USING (is_admin(auth.uid()::uuid));
...
-- View for flagged products
CREATE OR REPLACE VIEW flagged_products AS
SELECT 
  p.id,
  p.title,
  p.flag_reason,
