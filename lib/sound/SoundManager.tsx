      ...
      try {
        ...
        for (const sound of soundsToLoad) {
          ...
          try {
            ...
            console.warn(`Failed to load ${sound.key} from ${filePath}, trying default...`, error);
            // If custom file fails, try the default
            if (customAsset && filePath !== sound.defaultPath) {
              try {
                await soundManagerRef.current!.loadSound(sound.key, sound.defaultPath);
              } catch (fallbackError) {
                console.warn(`Failed to load default for ${sound.key}:`, fallbackError);
              }
            }
          }
        }
