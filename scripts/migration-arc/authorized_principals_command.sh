...
# Debug: show raw ssh-keygen -Lf output (helps debug parsing variants)
echo "DEBUG: SSH_LF_OUT_START"
echo "$SSH_LF_OUT"
echo "DEBUG: SSH_LF_OUT_END"
PR_LINE=$(echo "$SSH_LF_OUT" | grep -i -E '^\s*Principals:|^\s*Valid principals' -m1 || true)
# Debug: show matched PR_LINE
echo "DEBUG: PR_LINE=<$PR_LINE>"
PRINCIPALS_LINE=""
if [ -n "$PR_LINE" ]; then
